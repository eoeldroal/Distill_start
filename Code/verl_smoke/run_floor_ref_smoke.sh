#!/usr/bin/env bash
# relative-floor + anchor_from_ref: anchor를 reference policy 에서 채점한다.
# student+anchor GPU 2장 (같은 GPU), teacher GPU 1장. anchor 전용 GPU 없음.
set -xeuo pipefail
...
python3 -m verl.trainer.main_ppo \
  ...
  trainer.experiment_name=floor_ref_qwen3_1p7b \
  ...
  trainer.default_local_dir="${HERE}/ckpt_floor_ref" \
  "$@"
